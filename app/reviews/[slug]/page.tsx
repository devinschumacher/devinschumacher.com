...
export async function generateStaticParams() {
  ...
  // Filter for /reviews/* paths only
  return paths
    .filter(path => path.startsWith('/reviews/'))
    .map((path) => ({
      slug: path.replace('/reviews/', '').replace(/\/$/, ''),
    }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const slugPath = `/reviews/${params.slug}/`;
  ...
  return {
    title: `${post.meta.title} Review | ${siteConfig.name}`,
    description: post.meta.description,
    openGraph: {
      title: `${post.meta.title} Review`,
      description: post.meta.description,
      type: 'article',
...
export default function ReviewPostPage({ params }: PageProps) {
  const slugPath = `/reviews/${params.slug}/`;
  const contentPath = getContentPath(slugPath);
